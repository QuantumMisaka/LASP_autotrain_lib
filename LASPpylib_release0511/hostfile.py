...
def runprog_sbatch(workdir, script):
    '''in slurm-system use sbatch to parallelly running prog'''
    try:
        os.chdir(workdir)
        shutil.copy(script, workdir)
        mpiprog = "sbatch -J %s  %s"%(workdir, script)
        subprocess.call(mpiprog, shell=True, executable='bin/bash')
        return
